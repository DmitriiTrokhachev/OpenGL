...
int main(int argc, char *argv[]) {
    ...
    // Загрузка текстуры
    ImageData info = loadPngImage("/home/dima/OpenGLProj/OpenGL_Practice_FULL-template/res/test.png");
    uint textureId = 0;
    if(info.loaded){
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        modelViewProjMatrix = glm::rotate(modelViewProjMatrix, float(time), glm::normalize(vec3(0.5f, 0.0f, 0.5f)));
        ...
        //glGetUniformLocation(textureId, );
        ...
        //glDrawElements(GL_TRIANGLES, cubeVertexCount, GL_UNSIGNED_INT, NULL);
